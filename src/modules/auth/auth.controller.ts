...
export const loginWithEmailOrLogin = catchAsync(async (req: Request, res: Response) => {
  const { emailOrLogin, password } = req.body;
  const user = await authService.loginUserWithEmailOrLoginAndPassword(emailOrLogin, password);
  const tokens = await tokenService.generateAuthTokens(user);
...
export const sendVerifyEmail = catchAsync(async (user: IUserDoc) => {
  const verifyEmailToken = await tokenService.generateVerifyEmailToken(user);
  await emailService.sendVerificationEmail(user.email, verifyEmailToken, user.name);
});
